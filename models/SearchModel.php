<?php 
	trait SearchModel{
		...
		public function modelRead($recordPerPage){
			...
			$query = $conn->query("select * from products where name like '%$key%' or description like '%$key%' order by id desc limit $from,$recordPerPage");
		...
		public function modelSmartSearch(){
			...
			$conn = Connection::getInstance();
			//thuc hien truy van
			$query = $conn->prepare("select name,photo,id from products where name like '%$key%' or description like '%$key%'");
			//thuc thi truy van. Neu khong co tham so o cau truy van thi ghi array rong
			$query->execute(array());
